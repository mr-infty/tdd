...
data GameCmd : (ty : Type) -> GameState -> (ty -> GameState) -> Type where
  ...
  (>>=) : GameCmd a state1 state2_fn -> ((res : a) -> GameCmd b (state2_fn res) state3_fn) ->
          GameCmd b state1 state3_fn

namespace Loop
  data GameLoop : (ty : Type) -> GameState -> (ty -> GameState) -> Type where
    (>>=) : GameCmd a state1 state2_fn -> ((res : a) -> GameLoop b (state2_fn res) state3_fn) ->
...
Show (Game g) where
  ...
  show (GameWon word) = "Game won: word was " ++ show word
  show (GameLost word) = "Game lost: word was " ++ show word
...
removeElem {n = (S k)} value (y :: ys) {prf = (There later)} = y :: removeElem value ys {prf = later}

readGuess : (fuel : Fuel) -> (state : Game instate) -> IO (GameResult Char (const instate))
readGuess Dry _ = pure OutOfFuel
readGuess (More fuel) state = do putStr "Guess: "
                                 input <- getLine
                                 case (unpack input) of
                                       [x] => if isAlpha x
                                                 then ok (toUpper x) state
                                                 else do putStrLn "Invalid input"
                                                         readGuess fuel state
...
runCmd fuel (InProgress word (S guesses) missing) (Guess c) = case isElem c missing of
...
runCmd fuel state ShowState = do printLn state
                                 ok () state
runCmd fuel state (Message msg) = do putStrLn msg
                                     ok () state
runCmd fuel state ReadGuess = readGuess fuel state
